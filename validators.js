...
const validators = {
        ...
        validate: function(data) {
            ...
            for (let i = 0; i < entityLines.length; i++) {
                ...
                for (const attr of attributes) {
                    // Extract the attribute name without any markers (^PK, ^FK, [derived], (enums))
                    const baseAttrMatch = attr.match(/^([a-zA-Z0-9]+)(\^PK|\^FK|\[derived\]|\s*\(.*\))?/);
    ...
    V062: {
        ...
        validate: function(data) {
            ...
            const businessRuleBlocks = output.match(/## [a-z_]+_\d+[^#]+/g) || [];
            
            const missingValidationTrigger = [];
            
            for (const block of businessRuleBlocks) {
                if (!block.includes('*Validation:') && !block.includes('*Trigger:')) {
                    const functionName = block.match(/## ([a-z_]+_\d+)/);
                    if (functionName && functionName[1]) {
                        missingValidationTrigger.push(functionName[1]);
                    } else {
                        missingValidationTrigger.push('Unknown function');
                    }
                }
            }
