...
const validators = {
        ...
        validate: function(data) {
            ...
            for (let i = 0; i < entityLines.length; i++) {
                ...
                for (const attr of attributes) {
                    // Separate attribute name from enum values (if any)
                    const attributeNamePart = attr.split('(')[0].trim();
                    
                    // Extract the attribute name without any markers (^PK, ^FK, [derived])
                    const baseAttrMatch = attributeNamePart.match(/^([a-zA-Z0-9]+)(\^PK|\^FK|\[derived\])?/);
                    
                    if (baseAttrMatch && baseAttrMatch[1]) {
                        const attrName = baseAttrMatch[1];
                        // Skip empty, has, and annotations - also skip if the original attr was just an enum like "(Value)"
                        if (!attrName || attrName === 'has' || attributeNamePart === '') continue;
    ...
    V062: {
        ...
        validate: function(data) {
            ...
            // Extract business rule blocks
            const businessRuleBlocks = output.match(/##\s+[a-z_]+_\d+[\s\S]*?(?=\n##\s+|$)/g) || [];
            
            const missingValidationTrigger = [];
            
            for (const block of businessRuleBlocks) {
                if (!block.includes('*Validation:') && !block.includes('*Trigger:')) {
                    const functionNameMatch = block.match(/##\s*([a-z_]+_\d+)/) || block.match(/##\s*([a-zA-Z_][a-zA-Z0-9_]*)/);
                    if (functionNameMatch && functionNameMatch[1]) {
                        missingValidationTrigger.push(functionNameMatch[1]);
                    } else {
                        missingValidationTrigger.push('Unknown function (parsing error)');
                    }
                }
            }
    ...
    V090: {
        ...
        validate: function(data) {
            ...
            // Extract attribute additional properties blocks with a more specific pattern
