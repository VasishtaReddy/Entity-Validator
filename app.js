document.addEventListener('DOMContentLoaded', function() {
    ...
    function runValidation() {
        ...
        } else if (validationTypeSelect.value === 'lo') {
            const loResults = runLOValidators(jsonData);
            const passCount = loResults.filter(r => r.passed).length;
            const failCount = loResults.length - passCount;
            validationResult = {
                validationResults: loResults,
                summary: {
                    total: loResults.length,
                    pass: passCount,
                    fail: failCount,
                    passRate: loResults.length > 0 ? Math.round((passCount / loResults.length) * 100) : 100
                }
            };
        } else {
